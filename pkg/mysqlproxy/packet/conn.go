...
func NewConn(conn net.Conn) *Conn {
	c := new(Conn)
	c.Conn = conn
	// remove buffering to reduce memory footprint
	// c.bufPool = NewBufPool()
	// c.br = bufio.NewReaderSize(c, defaultReaderSize) // 8kb
	// c.reader = c.br
	c.reader = conn
	c.copyNBuf = utils.ByteSliceGet(defaultReaderSize)
	// c.copyNBuf = make([]byte, defaultReaderSize/4)
